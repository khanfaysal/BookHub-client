...
//       );
//       const lastUpdatedBooks = sortedBooks.slice(0, 10);
//       dispatch(setLastUpdatedBooks(lastUpdatedBooks));
//     }
//   }, [data, dispatch]);
...
function Books() {
  ...
  const [genreFilter, setGenreFilterLocal] = useState('');
  const [yearFilter, setYearFilterLocal] = useState('');
  const { genres, years, lastUpdatedBooks } = useSelector((state: RootState) => state.book);
